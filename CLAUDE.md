...
- **Output location is configured or inferred.** `PluginConfiguration.PlaylistPath` wins if set; otherwise `PlaylistLocator` takes three `Path.GetDirectoryName` steps up from any track's path (assuming `<root>/<artist>/<album>/<track>`) and appends `playlists/`. Export, import and the config page all go through it, so the convention lives in exactly one place — keep it that way. It seeds from the first track in the *library* rather than the first *favorited* one, so the folder still resolves on a server with no favorites yet, which is the state a fresh import machine is in.
- **A read-only media folder is a supported state, not a crash.** `TryPrepareDirectory` creates the folder *and* writes a probe file before the export starts — creating a directory can succeed on a share that then refuses the files — and returns a message the dashboard shows. Network-mounted libraries (CIFS especially) routinely present as writable locally while the remote server denies the write.
- **Favorites are merged across all users** into a single set of playlists, despite the task description saying "for each user".
- **Aggregate files are scoped to the server, per-album ones are not.** Several servers can point at one media folder, so `Favorites-<ServerName>.json` and `Favorites-<ServerName>.m3u` carry the server name — always, even with one server, so ownership never depends on who exported first. `<Artist> - <Album>.m3u` is deliberately unscoped: its contents follow from the album, not from who wrote it, so two servers producing it is harmless. Each manifest also carries `ServerId` (`IApplicationHost.SystemId`), which survives a rename.
- **The import manifest name arrives over HTTP and is hostile until proven otherwise.** `TryResolveManifest` rejects anything that is not a plain `Favorites*.json` resolving inside the playlists folder — a bare filename parameter is still a traversal vector.
